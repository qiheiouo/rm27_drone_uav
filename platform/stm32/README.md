...
`nav_swarm_frame_read` 和 `nav_swarm_frame_write` 收发 48 字节版本化邻机状态帧。
`NavApp` 负责 CRC/数值检查、重复与倒序拒绝、接收时间计龄和超时淘汰；板级不应直接
拼装 `SwarmView` 或发送 C 结构体。默认每周期最多接收 4 帧、每 5 个周期发送一帧，
可分别覆盖 `NAV_APP_SWARM_RX_BUDGET` 和 `NAV_APP_SWARM_TX_PERIOD_STEPS`。详细格式与
带宽说明见[集群链路协议](../../docs/swarm_link.md)。默认 `SWARM_DISABLED` 时不会调用
这些收发接口；只有经过配置审查后才能启用。

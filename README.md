...
| 多机扩展 | 固定 48 字节版本化状态帧、CRC、重复/乱序/超时邻机表，最多 4 个他机状态、未来冲突检测及按 `agent_id` 确定性让行；默认可关闭 |
...
- 17 个模块级测试（含共享运行时、输入时效、watchdog、事件环、故障注入器、遥测编解码、集群链路与 STM32 非阻塞收发、失效轨迹及蜂群安全链路）；
- 8 个端到端故障回归场景；
...
进一步说明见 [架构文档](docs/architecture.md)、[集群链路](docs/swarm_link.md)、[日志回放与 watchdog](docs/replay_and_watchdog.md)、[总体方案](General_Plan.md) 和 [STM32 移植说明](platform/stm32/README.md)。

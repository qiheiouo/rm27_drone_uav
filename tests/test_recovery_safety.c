...
static SafetyDecision update_safety(SafetyMonitor *monitor, NavState *nav,
                                    CollisionRiskLevel risk, float dt)
{
    SafetyInput input;
    input.nav = nav;
...
int main(void)
{
    ...
    for (index = 0; index < 4; index++) {
        decision = update_safety(&safety, &nav, COLLISION_RISK_CRITICAL, 0.01f);
    }
    ...
    safety_init(&safety, &scenario.safety);
    for (index = 0; index < 4; index++) {
        decision = update_safety(&safety, &nav, COLLISION_RISK_NONE, 0.01f);
    }
